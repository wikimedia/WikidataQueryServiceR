#' @title Send a SPARQL query to WDQS
...
query_wikidata <- function(sparql_query, format = c("simple", "smart"), ...) {
  ...
  if (format[1] == "simple") {
    ...
    if (httr::http_type(response) == "text/csv") {
      ...
      output <- utils::read.csv(con, header = TRUE, stringsAsFactors = FALSE)
      message(nrow(output), " rows were returned by WDQS")
      return(output)
    } else {
    ...
    if (httr::http_type(response) == "application/sparql-results+json") {
      temp <- jsonlite::fromJSON(httr::content(response, as = "text", encoding = "UTF-8"),
                                 simplifyVector = FALSE)
    }
    httr::stop_for_status(response)
    if (length(temp$results$bindings) > 0) {
      output <- dplyr::bind_rows(lapply(temp$results$bindings, function(x) {
      ...
      if (any(datetime_cols)) {
        for (datetime_col in which(datetime_cols)) {
          output[[datetime_col]] <- as.POSIXlt(output[[datetime_col]], format = "%Y-%m-%dT%H:%M:%SZ", tz = "GMT")
        }
      }
      message(nrow(output), " rows were returned by WDQS")
      return(output)
